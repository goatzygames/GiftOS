...
// --- Admin login & admin actions (fix for missing renderAdminLogin & runMatchingAlgorithm) ---
async function renderAdminLogin(eventId) {
    // show simple inline admin login / control panel
    const docRef = db.collection('events').doc(eventId);
    const doc = await docRef.get();
    ...
    contentDiv.innerHTML = `
        ...
        <p>${t('adminText')}</p>
        <input type="password" id="adminPass" placeholder="${t('hostPass')}" />
        <button id="adminLoginBtn">${t('login')}</button>
        <button class="secondary" onclick="checkEventStatus('${eventId}')">${t('cancel')}</button>
        <div id="admin-controls" style="margin-top:12px; display:none;">
            <p><strong>${t('eventSettings')}</strong></p>
            <button id="closeShuffleBtn">${t('closeShuffle')}</button>
            <button id="exportCsvBtn">${t('exportCSV')}</button>
            <button id="exportJsonBtn">${t('exportJSON')}</button>
            <button id="resetPartsBtn">${t('resetParticipants')}</button>
            <button id="deleteEventBtn" style="color:#b00;">${t('deleteEvent')}</button>
            <div id="admin-msg" style="margin-top:8px;color:var(--muted)"></div>
        </div>
    `;

    document.getElementById('adminLoginBtn').addEventListener('click', async () => {
        const pass = (document.getElementById('adminPass').value || '').trim();
        if (!pass) return alert(t('fillFields'));
        // compare to stored hostPass (plaintext in this app)
        if (pass !== data.hostPass) return alert(t('incorrectPass'));
        // show admin controls
        document.getElementById('admin-controls').style.display = 'block';
        showToast(t('adminText'));
    });

    // attach control handlers
    document.getElementById('closeShuffleBtn').addEventListener('click', async () => {
        if (!confirm(t('closeShuffle') + '?')) return;
        try {
            await runMatchingAlgorithm(eventId, /*auto*/ false);
            showToast(t('pairsMade'));
            // refresh view
            checkEventStatus(eventId);
        } catch (e) {
            console.error(e);
            alert('Matching failed: ' + (e.message || e));
        }
    });

    document.getElementById('exportCsvBtn').addEventListener('click', async () => {
        const csv = await exportParticipantsCSV(eventId);
        downloadText(csv, `${eventId}-participants.csv`, 'text/csv');
    });

    document.getElementById('exportJsonBtn').addEventListener('click', async () => {
        const json = await exportParticipantsJSON(eventId);
        downloadText(JSON.stringify(json, null, 2), `${eventId}-participants.json`, 'application/json');
    });

    document.getElementById('resetPartsBtn').addEventListener('click', async () => {
        if (!confirm(t('resetParticipants') + '?')) return;
        await resetParticipantsAssignments(eventId);
        showToast(t('saveChanges'));
        checkEventStatus(eventId);
    });

    document.getElementById('deleteEventBtn').addEventListener('click', async () => {
        if (!confirm(t('confirmRemove') + ' event?')) return;
        try {
            await deleteEventAndParticipants(eventId);
            showToast(t('deleteEvent'));
            // go back to landing
...
async function exportParticipantsJSON(eventId) {
    const partsSnap = await db.collection('events').doc(eventId).collection('participants').get();
    const arr = [];
    partsSnap.forEach(d => arr.push({ id: d.id, ...d.data() }));
    return arr;
}
async function exportParticipantsCSV(eventId) {
    const arr = await exportParticipantsJSON(eventId);
